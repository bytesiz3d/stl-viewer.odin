...
@(private="file") atlas_texture: u32
@(private="file") atlas_quad_mesh: Quad_Mesh
@(private="file")
_load_atlas_texture :: proc() {
	// texture
...
r_init :: proc() -> (ok: bool) {
	...
	atlas_quad_mesh = quad_mesh_new()

	return true
}

r_dispose :: proc() {
	quad_mesh_free(&atlas_quad_mesh)
	gl.DeleteTextures(1, &atlas_texture)
}
...
r_process_commands:: proc(ctx: ^mu.Context) {
	cmd: ^mu.Command;
	for mu.next_command(ctx, &cmd) {
		#partial switch c in cmd.variant {
		case ^mu.Command_Text: _draw_text(c.str, c.pos, c.color)
		case ^mu.Command_Rect: _draw_rect(c.rect, c.color)
		case ^mu.Command_Icon: _draw_icon(c.id, c.rect, c.color)
		case ^mu.Command_Clip: _set_clip_rect(c.rect)
		case ^mu.Command_Jump: unreachable() // handled internally by next_command()
		}
	}
}

r_process_events :: proc(ctx: ^mu.Context, event_queue: ^Event_Queue) {
	// Key
	map_key :: proc(glfw_key: i32) -> (key: mu.Key, ok: bool) {
		switch glfw_key {
	...
	map_mouse :: proc(glfw_btn: i32) -> (btn: mu.Mouse, ok: bool) {
		switch glfw_btn {
	...
	for i in 0..<queue.len(event_queue^) {
		switch ev in queue.get(event_queue, i) {
		case Event_Key:
			switch ev.action {
			case .DOWN: if k, ok := map_key(ev.key); ok do mu.input_key_down(ctx, k)
			case .UP:   if k, ok := map_key(ev.key); ok do mu.input_key_up(ctx, k)
			}

		case Event_Char:
			bytes, sz := utf8.encode_rune(ev.r)
			mu.input_text(ctx, string(bytes[:sz]))

		case Event_Mouse_Button:
			switch ev.action {
			case .DOWN: if b, ok := map_mouse(ev.button); ok do mu.input_mouse_down(ctx, i32(g_input.mouse_pos.x), i32(g_input.mouse_pos.y), b)
			case .UP:   if b, ok := map_mouse(ev.button); ok do mu.input_mouse_up(ctx,   i32(g_input.mouse_pos.x), i32(g_input.mouse_pos.y), b)
			}

		case Event_Mouse_Move:
			mu.input_mouse_move(ctx, i32(ev.pos.x), i32(ev.pos.y))

		case Event_Mouse_Scroll:
			mu.input_scroll(ctx, 0, i32(ev.delta) * -30) // TODO: Verify
...
@(private = "file")
_draw_icon :: proc(id: mu.Icon, rect: mu.Rect, color: mu.Color) {
	src := atlas[int(id)]
	x := rect.x + (rect.w - src.w) / 2
	y := rect.y + (rect.h - src.h) / 2
	_atlas_quad({x, y, src.w, src.h}, src, color)
}

@(private = "file")
_draw_rect :: proc(rect: mu.Rect, color: mu.Color) {
	_atlas_quad(rect, atlas[ATLAS_WHITE], color)
}
...
_draw_text :: proc(text: string, pos: mu.Vec2, color: mu.Color) {
	...
	for ch in text {
		...
		dst.w = src.w
		dst.h = src.h
		_atlas_quad(dst, src, color)
		dst.x += dst.w
	}
}

@(private = "file")
_set_clip_rect :: proc(rect: mu.Rect) {
	// TODO: Scissor?
}

@(private = "file")
_atlas_quad :: proc(dst, src: mu.Rect, color: mu.Color) {
	// uniforms
	shader_atlas_update_uniforms(shader_atlas, {f32(color.r), f32(color.g), f32(color.b), f32(color.a)} / 256)
	...
	// draw call
	quad_mesh_draw(atlas_quad_mesh, 
		{f32(src.x) / ATLAS_WIDTH, f32(src.y) / ATLAS_HEIGHT, f32(src.x + src.w) / ATLAS_WIDTH, f32(src.y + src.h) / ATLAS_HEIGHT},
		{f32(dst.x), f32(dst.y), f32(dst.x + dst.w), f32(dst.y + dst.h)},
	)
}

ATLAS_WIDTH          :: 128
ATLAS_HEIGHT         :: 128
ATLAS_DIM            :: [2]f32{ATLAS_WIDTH, ATLAS_HEIGHT}
ATLAS_WHITE          :: cast(int) len(mu.Icon) + 1
ATLAS_FONT           :: cast(int) len(mu.Icon) + 2
ATLAS_ICON_CLOSE     :: cast(int) mu.Icon.CLOSE
ATLAS_ICON_CHECK     :: cast(int) mu.Icon.CHECK
ATLAS_ICON_EXPANDED  :: cast(int) mu.Icon.EXPANDED
...
atlas := [?]mu.Rect{
	...
	ATLAS_ICON_COLLAPSED = { 113, 68, 5,  7 },
	ATLAS_ICON_RESIZE    = { 104, 68, 9, 9 },
